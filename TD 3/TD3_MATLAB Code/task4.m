...
if time < my_alg('t_finish')    % Check for algorithm finish time
    ...
    if dt>my_alg('sampling_outer')  % Execute code when desired outer loop sampling time is reached
        my_alg('t_outer_loop') = tic;
% Calculate line following PID value, in angular speed(wR, wL)
        ...
        %scanning the wall 
        my_alg('sonar_dist') = my_alg('sonar');
        if my_alg('sonar_dist')>2 && my_alg('servo motor') == 1.57
            my_alg('servo motor') = 0;
        elseif my_alg('sonar_dist')>2 && my_alg('servo motor') == 0
            my_alg('servo motor') = -1.57;
        elseif my_alg('sonar_dist')>2 && my_alg('servo motor') == -1.57
            my_alg('servo motor') = 1.57;
        end
        ...
        if (my_alg('sensor_4') == 500 || my_alg('sensor_5') == 500)
            my_alg('wR_desired')=my_alg('wR_desired_temp')/w_max;
            my_alg('wL_desired')=my_alg('wL_desired_temp')/w_max;
            my_alg('count') = 0;
            
            % if the right middle sensors above the line
        elseif (my_alg('sensor_5') == 500 || my_alg('sensor_6') == 500)
            my_alg('wR_desired')=(my_alg('wR_desired_temp')/w_max - my_alg('PID'));
            my_alg('wL_desired')=(my_alg('wL_desired_temp')/w_max + my_alg('PID'));
            my_alg('count') = 0;
        ...
        elseif (my_alg('sensor_7') == 500 || my_alg('sensor_8') == 500)
            my_alg('wR_desired')=(my_alg('wR_desired_temp')/w_max - my_alg('PID'))*0.8;
            my_alg('wL_desired')=(my_alg('wL_desired_temp')/w_max + my_alg('PID'))*0.8;
            my_alg('count') = 0;
        ...
        elseif (my_alg('sensor_1') == 500 || my_alg('sensor_2') == 500)
            ...
            % All the sensors detected nothing
        elseif my_alg('sensor_1') == 2500 && my_alg('sensor_2') == 2500 && my_alg('sensor_3') == 2500 && my_alg('sensor_4') == 2500 && my_alg('sensor_5') == 2500 && my_alg('sensor_6') == 2500 && my_alg('sensor_7') == 2500 && my_alg('sensor_8') == 2500 && my_alg('sonar_dist')>2 
            my_alg('wR_desired')=my_alg('wR_desired_temp')/w_max;
            my_alg('wL_desired')=my_alg('wL_desired_temp')/w_max;
            my_alg('count')=my_alg('count')+1;
            
            %if the sonar snesor detecting the wall we contorl it
            %according to the sonar
        elseif my_alg('sonar_dist')<2 && my_alg('servo motor')==1.57
            if my_alg('sonar_dist')<1
                my_alg('wR_desired')= (my_alg('wR_desired_temp')-(1-my_alg('sonar_dist'))*4)/w_max;
                my_alg('wL_desired')= (my_alg('wL_desired_temp')+(1-my_alg('sonar_dist'))*4)/w_max;
            elseif my_alg('sonar_dist')>1
                my_alg('wR_desired')= (my_alg('wR_desired_temp')+(my_alg('sonar_dist')-1)*4)/w_max;
                my_alg('wL_desired')= (my_alg('wL_desired_temp')-(my_alg('sonar_dist')-1)*4)/w_max;  
            end
        elseif my_alg('sonar_dist')<2 && my_alg('servo motor')==-1.57
            if my_alg('sonar_dist')<1
                my_alg('wR_desired')= (my_alg('wR_desired_temp')+(1-my_alg('sonar_dist'))*4)/w_max;
                my_alg('wL_desired')= (my_alg('wR_desired_temp')-(1-my_alg('sonar_dist'))*4)/w_max;
            elseif my_alg('sonar_dist')>1
                my_alg('wR_desired')= (my_alg('wR_desired_temp')-(my_alg('sonar_dist')-1)*4)/w_max;
                my_alg('wL_desired')= (my_alg('wL_desired_temp')+(my_alg('sonar_dist')-1)*4)/w_max;  
            end
%         elseif my_alg('sonar_dist')<0.7 && my_alg('servo motor')==0 
%             if my_alg('sonar_dist')<1
%                 my_alg('wR_desired')= 0;
%                 my_alg('wL_desired')= 0;
%             end
        ...
        % The speed control for the extreme case and final stop
        if my_alg('count') >= 10 && my_alg('count') < 2000
             my_alg('wR_desired') = my_alg('wR_desired_temp')/w_max;  
             my_alg('wL_desired') = my_alg('wL_desired_temp')/w_max;
        end
        if my_alg('count') >= 2000 || my_alg('sonar_dist')<0.7 && my_alg('servo motor')==0 
             my_alg('wR_desired') = 0;  
             my_alg('wL_desired') = 0;
        end
        
        % give the motor desired speed
        my_alg('right motor')=my_alg('wR_desired');
        my_alg('left motor')=my_alg('wL_desired');
...
% %         end
% 
% %This part is designed to control the speed using pid controller
% %         my_alg('Rerror') = (my_alg('wR_desired') - my_alg('right encoder')/w_max );
% %         my_alg('RPID') = my_alg('R_kp')*my_alg('Rerror') +  my_alg('R_ki')*my_alg('Rtot_error') + my_alg('R_kd')*my_alg('Rdiff_error');
% %         my_alg('RPID1')=my_alg('RPID');
% %         % Previous error
...
%         
%         if my_alg('right encoder')>my_alg('wR_cutoff') && my_alg('left encoder')>my_alg('wL_cutoff')
%             my_alg('RPID') = my_alg('RPID1')*0.1;
%             my_alg('LPID') = my_alg('LPID1')*0.1;
%         elseif my_alg('right encoder')<4 && my_alg('left encoder')<4
