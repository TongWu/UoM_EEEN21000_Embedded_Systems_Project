...
public:
    ...
    double cal()
    {
        if(sensor.GetSensorValues(4) == 500 || sensor.GetSensorValues(5) == 500)
            {error = 0;}
        else {error = abs(sensor.readsensor());} // Function readsensor() should be reflectance() in MATLAB

        sensor_output[] = sensor_reading();
        ...
        else if(sensor.GetSensorValues(1) == 2500 && sensor.GetSensorValues(2) == 2500 && sensor.GetSensorValues(3) == 2500 && sensor.GetSensorValues(4) == 2500 && sensor.GetSensorValues(5) == 2500 && sensor.GetSensorValues(6) == 2500 && sensor.GetSensorValues(7) = 2500 && sensor.GetSensorValues(8) == 2500)
        {
            R.MotorWrite(wR_desired/w_max);
            L.MotorWrite(wL_desired/w_max);
            count += 1;
...
double sensor_reading()
{
    ...
    for(int i=0;i<sensor.GetSensorCount();i++)
    {
        ...
        reflectance_raw[i] = sensor.GetSensorCount(i);
        ...
        reflectance += reflectance_raw[i] * (((-15)/48000)+(i+1)(1/8000));
    }
    // Set output as an array, the last content is the reflectance, all previous data are reflectance_raw
    return sensor_output[reflectance_raw, reflectance];
}
