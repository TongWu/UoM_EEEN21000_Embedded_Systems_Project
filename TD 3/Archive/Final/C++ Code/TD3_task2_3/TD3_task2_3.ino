...
public:
    ...
    double cal()
    {
        ...
        for (int i = 0; i < sensor.GetSensorCount(); i++)
        {
            ...
            // Assume the minimum reflectance is -10, produced by sensor_5 and sensor_6 reading are 500, all others are 2500
            // Maximum reflectance is 10
            // The calculated error_constant for sensor_1 is -15/48000, -9/48000 for sensor_2, ... 
            
            // todo>2021.4.20 The calculated error_constantis not verified and need to tunning
            // todo>2021.4.20 Error Constant should be updated
...
void loop()
{
    ...
    auto millis_ = millis();
    auto millis_pre;
    // Set the sampling time to 0.005 and toc.
    if (millis_ - millis_pre < 0.5)
    {
        millis_pre = millis();
